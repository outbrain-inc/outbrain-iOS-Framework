...
@interface OBKeychainStore () {
    NSMutableDictionary *itemsToUpdate;
}

@end

@implementation OBKeychainStore
...
{
    return [self removeItemForKey:key service:[self defaultService] accessGroup:nil];
}

+ (BOOL)removeItemForKey:(NSString *)key service:(NSString *)service
{
    return [self removeItemForKey:key service:service accessGroup:nil];
}
...
{
    NSArray *items = [[self class] itemsForService:self.service accessGroup:self.accessGroup];
